...
def TakeImages():        
    ...
    if(is_number(Id)):
        ...
        harcascadePath = "D:\Programming\Python\DeepLearning\haarcascade_frontalface_default.xml"
        ...
        while(True):
            ...
            for (x,y,w,h) in faces:
                ...
                cv2.imwrite("D:\Programming\Python\DeepLearning\TrainingImage\Train.User."+Id +'.'+ str(sampleNum) + ".jpg", gray[y:y+h,x:x+w])
                #display the frame
...
def TrainImages():
    ...
    harcascadePath = "D:\Programming\Python\DeepLearning\haarcascade_frontalface_default.xml"
    detector=cv2.CascadeClassifier(harcascadePath)
    TrainingImagePath="D:\Programming\Python\DeepLearning\TrainingImage"
    faces,Ids = getImagesAndLabels(TrainingImagePath)
    recognizer.train(faces, np.array(Ids))
    recognizer.save("D:\Programming\Python\DeepLearning\TrainingImageLabel\Trainner.yml")
    res = "Image Trained" 
    message.configure(text= res)
...
def TrackImages():
    recognizer = cv2.face.LBPHFaceRecognizer_create()#cv2.createLBPHFaceRecognizer()
    recognizer.read("D:\Programming\Python\DeepLearning\TrainingImageLabel\Trainner.yml")
    harcascadePath = "D:\Programming\Python\DeepLearning\haarcascade_frontalface_default.xml"
    faceCascade = cv2.CascadeClassifier(harcascadePath);    
    df=pd.read_csv("D:\Programming\Python\DeepLearning\StudentDetails\StudentDetails.csv")
    cam = cv2.VideoCapture(0)
    ...
    while True:
        ...
        for(x,y,w,h) in faces:
            ...
            if(conf < 50):
                ...
                tt=str(Id)  
            if(conf > 75):
                noOfFile=len(os.listdir("D:\Programming\Python\DeepLearning\ImagesUnknown"))+1
                cv2.imwrite("D:\Programming\Python\DeepLearning\ImagesUnknown\Image"+str(noOfFile) + ".jpg", im[y:y+h,x:x+w])            
    ...
    fileName="D:\Programming\Python\DeepLearning\Attendance\Attendance_"+date+"_"+Hour+"-"+Minute+"-"+Second+".csv"
